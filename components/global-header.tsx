        ...
        <div className="flex items-center justify-between h-14">
          ...
          <div className="flex items-center gap-1 md:gap-2">
            ...
            <div className="relative group">
              <button className="flex items-center gap-1.5 px-3 py-2 text-sm font-medium text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg transition-colors">
                <Languages className="h-4 w-4" />
                <span className="hidden sm:inline">{localeNames[locale]}</span>
              </button>

              {/* Language dropdown */}
              <div className="absolute right-0 mt-2 w-40 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                {otherLocales.map((l) => (
                  <Link
                    key={l}
                    href={`/${l}/questions`}
            ...
            <LicenseCategorySelector locale={locale} />
          </div>
        </div>
      </div>
    </header>
